    ...
    try {
      ...
      if (file) {
        ...
        if (result.resume_data) {
          onAnalysisComplete?.(result.resume_data);
          onClose();
        } else {
          // If resume_data not in response, fetch it
          const { getResume } = await import('@/lib/api');
          const resumeData = await getResume(result.resume_id);
          onAnalysisComplete?.(resumeData.parsed_data);
          onClose();
        }
      } else if (resumeText) {
        // For text-only, we'd need a different endpoint
        // For now, show error that PDF upload is required
